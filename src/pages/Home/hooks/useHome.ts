...
export function useHome() {
  ...
  useEffect(() => {
    ...
    fetch('https://api.quotable.io/random?tags=technology,famous-quotes')
      ...
      .catch(() => {
        // Fallback: Silently ignore CORS or rate limits and stick to the local fallback
      });
  }, []);

  const stats = [
    { label: "Projects Built", count: "50+", icon: FiFolder, color: "text-brand-accent-secondary bg-brand-accent-secondary/10 border-brand-accent-secondary/20" },
